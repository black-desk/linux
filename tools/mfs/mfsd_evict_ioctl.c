 ...
 *
 * Example use:
 * ./mfsd_evict_ioctl [mfs_mountpoint] [mtree_path]
 * mfsd_evict_ioctl demonstrates how to evict page cache when
 * file is read to end using MFS_IOC_EVICT ioctl, which is
 * more efficient than posix_fadvise because it operates
 ...
 *
 * This demo is identical to mfsd_evict_simple, but uses ioctl
...
static struct file_track *get_file_track(int mfs_fd)
{
	...
	for (i = 0; i < MAX_FILES; i++) {
		if (!files[i].valid) {
			...
			pr_err("[TRACK] mfs_fd=%d path=%s size=%zu\n",
			       mfs_fd, path, files[i].size);
			return &files[i];
		}
	}
...
/* Evict file page cache using MFS_IOC_EVICT ioctl */
...
static int process_read(struct mfs_msg *msg)
{
	...
	if (track->size > 0 && track->last_off >= track->size) {
		if (evict_queue_count < MAX_FILES) {
			...
			evict_queue[slot].mfs_fd = track->mfs_fd;
			...
			evict_file(track->mfs_fd, track->path, "read to end (queue full)");
			free(track->path);
		}
		track->valid = 0;
		current_mfs_fd = -1;
...
int main(int argc, char *argv[])
{
	...
	if (argc != 3) {
		printf("./mfsd_evict_ioctl ${mfs_mountpoint} ${mtree_path}\n");
		return -1;
	}
	mountpoint = argv[1];
	mtree_path = argv[2];
